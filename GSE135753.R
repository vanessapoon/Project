...
list.files ("GSE135753_LPS/Salmon") 
salmon.files <- list.files ("GSE135753_LPS/Salmon", full=TRUE)
salmon.files

read.delim ("GSE135753_LPS/SRA.tabular")
samples <- read.delim("GSE135753_LPS/SRA.tabular")
samples

samples %>% arrange(Run) 
samples <- samples %>% arrange(Run) %>% dplyr::select("Run", "source_name") 
row.names(samples) <- samples$Run
samples

samples$Treatment <- c("LPS", "Control", "LPS", "Control", "LPS", "Control")
samples

library(tximport)
library(GenomicFeatures)

samples$source_name <- as.factor(samples$source_name)
samples$source_name <- factor(samples$source_name, labels = c("macrophage"))

samples$group <- paste(samples$source_name, samples$Treatment, sep = ".")
samples
...
#only need to run these two lines once to create reuslts_all object and save as RDS file
# results_all <- list()
# saveRDS(results_all, file = "results_all.RDS")

results_all <- readRDS("results_all.RDS")
results_all[["GSE135753_LPS"]] <- res
