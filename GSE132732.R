...
list.files ("GSE132732_IL4/Salmon") 
salmon.files <- list.files ("GSE132732_IL4/Salmon", full=TRUE)
salmon.files

read.delim ("GSE132732_IL4/SRA.tabular")
samples <- read.delim("GSE132732_IL4/SRA.tabular")
samples
...
dds <- DESeq(dds)
results(dds, contrast = c("group", "macrophage.IL4", "macrophage.Control")) %>%
  as.data.frame() %>%
  filter(padj<0.05)
...
res <- results(dds, contrast = c("group", "macrophage.IL4", "macrophage.Control")) %>%
  as.data.frame() %>%
